...
def demo_xlsx(tmp_path):
    ...
    ws1 = wb.active
    ws1.title = "登录测试"
    ws1.append(["keyword", "name", "locator", "text", "url", "expected"])
    ws1.append(["navigate", "打开登录页", None, None, "https://example.com/login", None])
    ws1.append(["check", "验证页面加载", "#login-form", None, None, None])
    ws1.append(["type", "输入用户名", "#username", "admin", None, None])
    ws1.append(["type", "输入密码", "#password", "password123", None, None])
    ws1.append(["click", "点击登录", "#login-btn", None, None, None])
    ws1.append(["wait", "等待跳转", None, None, None, None])
    ws1.append(["assert", "验证登录成功", "#welcome", None, None, "欢迎"])

    ws2 = wb.create_sheet("搜索测试")
    ws2.append(["keyword", "name", "locator", "text", "value"])
    ws2.append(["navigate", "打开首页", None, None, None])
    ws2.append(["type", "输入搜索词", "#search", "RodSki", None])
    ws2.append(["click", "点击搜索", "#search-btn", None, None])
    ws2.append(["select", "选择排序", "#sort", None, "newest"])
    ws2.append(["scroll", "向下滚动", None, None, None])
    ws2.append(["hover", "悬停结果", ".result:first", None, None])
