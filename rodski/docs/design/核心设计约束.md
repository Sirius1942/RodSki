...
`assert` 是用于**视觉匹配**的关键字，通过截图/图片比对或视频帧分析判断用例是否完成。

**定义**：
- 对当前页面/应用截图，与预期图片进行匹配度比对
- 对执行录屏进行帧分析，验证预期画面是否出现
- 用 `type` 参数区分断言类型，不新增独立关键字
...
| `wait` | int | 否 | 等待图片出现的最大秒数，默认 0，仅 `type=image` 生效 |
...
| `position` | string | 否 | 帧位置：`any`/`start`/`end`/`middle`，默认 `any`，仅 `type=video` 生效 |
...
```python
# type=image
{"matched": true, "similarity": 0.87, "threshold": 0.85, "screenshot": "path/to/actual.png", "reference": "img/expected_modal.png"}

# type=video
{"matched": true, "frame_found": true, "timestamp": 3.5, "similarity": 0.82, "total_frames": 300, "reference": "img/expected_frame.png"}
```

**约束（强制）**：
- ❌ **禁止出现** `image_assert`、`video_assert`、`picture_assert` 等独立关键字
- ✅ **必须使用** `type` 参数区分类型：`assert[type=image]` / `assert[type=video]`
- `verify` 回答"值对不对"，`assert` 回答"画面对不对"，职责正交
- 扩展性：未来可扩展 `type=audio`（声音断言）等，不破坏现有设计
