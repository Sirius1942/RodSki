...
| 类型 | 格式 | 说明 | 示例 |
|------|------|------|------|
| `vision` | 语义描述 | OmniParser + LLM 语义匹配 | `vision:登录按钮` |
| `vision_bbox` | 坐标 | 直接使用坐标 `x1,y1,x2,y2` | `vision_bbox:100,200,150,250` |

**视觉定位器约束**：
- `vision`: 通过自然语言描述定位，支持中英文
- `vision_bbox`: Web 用像素坐标，Desktop 用屏幕绝对坐标
- 无需 AI 调用，性能更高
