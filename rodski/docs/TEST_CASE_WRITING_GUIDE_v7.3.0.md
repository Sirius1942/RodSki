...
15. [iOS 自动化（v7.3.0）](#15-ios-自动化v730)
16. [附录：常见问题](#附录常见问题)
...
v7.3.0 在 Android 已有能力基础上，补全了 iOS 原生自动化支持。设计原则：

- **关键字层完全平台无关**：`type / verify / navigate / wait / close` 在 iOS 上语义与 Android 完全一致，case XML 无需修改
- **平台差异只在 model.xml 中承载**：通过 `driver_type` + 定位器 `platform` 属性区分
- **数据层完全复用**：`data.sqlite` 与平台无关，Android/iOS 共用同一份数据

### 15.2 新增 driver_type="mobile"

`driver_type="mobile"` 表示平台无关的移动端模型，运行时由 `--platform` 参数或 `globalvalue.xml` 中 `Mobile.Platform` 决定实际平台。

```xml
<!-- driver_type=mobile：同一 model 同时服务 Android 和 iOS -->
<model name="LoginScreen" type="ui" driver_type="mobile">
  ...
</model>
```

`driver_type` 完整枚举（v7.3.0）：`web` / `interface` / `database` / `windows` / `macos` / `android` / `ios` / `mobile`

### 15.3 定位器 platform 属性

`<location>` 元素新增可选 `platform` 属性，用于标记平台专属定位器：
...
- `platform` 不匹配当前平台 → 跳过
- 同 priority 下，平台匹配的 location 优先于通用 location

### 15.4 定位器跨平台能力

| 定位器 | 跨平台 | 说明 |
|--------|--------|------|
| `ocr` | ✅ 完全跨平台 | 两端通用，无需 platform 标记 |
| `text` | ✅ 语义跨平台 | driver 内部按平台构造不同 XPath |
| `vision` | ✅ 完全跨平台 | VLM/Accessibility Tree 语义匹配 |
| `vision_image` | ✅ 完全跨平台 | OpenCV 模板匹配 |
| `vision_bbox` | ⚠️ 注意分辨率 | 坐标因设备分辨率不同，建议标 platform |
| `id` | ❌ 平台专属 | Android: resource-id；iOS: accessibility-id |
| `xpath` | ❌ 平台专属 | Android/iOS 层级结构不同 |
| `class` | ❌ 平台专属 | Android: `android.widget.*`；iOS: `XCUIElementType*` |

### 15.5 iOS 专属定位器：predicate / class_chain

iOS 新增两个高性能定位器（必须标 `platform="ios"`）：

| 定位器 | 说明 | 示例 |
|--------|------|------|
| `predicate` | NSPredicate 字符串，精确匹配元素属性 | `label == '登录' AND type == 'XCUIElementTypeButton'` |
| `class_chain` | XCUITest 路径语法，比 xpath 快 | `**/XCUIElementTypeButton[\`label == '登录'\`]` |

```xml
<element name="loginBtn" type="mobile">
  <type>button</type>
  <location type="id"          platform="android" priority="1">com.rodski.demo:id/loginBtn</location>
  <location type="predicate"   platform="ios"     priority="1">label == '登录' AND type == 'XCUIElementTypeButton'</location>
...
元素全用 `ocr/text/vision_image` 定位，一个 model 两端共用：
...
<globalvalue>
  <group name="Mobile">
    ...
    <var name="AutomationBackend"  value="appium"/>
    <var name="AppiumServer"       value="http://127.0.0.1:4723"/>
    <var name="DeviceName"         value="iPhone 16"/>              <!-- Simulator 名称或真机名 -->
    <var name="UDID"               value="AC199BB6-54B6-424A-9D3C-B8CEA8DF89BC"/>  <!-- 可选但推荐 -->
...
|-----|------|
| `Platform` | 固定填 `ios` |
| `DeviceName` | Simulator 名称（如 `iPhone 16`）或真机名称 |
| `UDID` | 设备/Simulator UDID，多设备时避免歧义 |
| `BundleId` | iOS App 的 Bundle ID |
| `AppTarget` | `app://ios/{BundleId}` 格式，供 `navigate` 使用 |

### 15.8 平台切换方式

同一套 case + data，通过两种方式切换平台：
...
# 方式二：不同 globalvalue 文件（持久配置）
rodski run @ios_smoke    # plan 指向 globalvalue_ios.xml
rodski run @android_smoke
```
...
| 环节 | Simulator | 真机 |
|------|-----------|------|
| 连接方式 | 无需连接 | USB + WebDriverAgent (WDA) |
| 签名 | 无需 | 需要 Apple 开发者账号签名 WDA |
| 工具 | Xcode + Appium xcuitest driver | Xcode + xcodebuild + WDA |
| SDK | 与 Xcode 版本匹配 | 需要对应 iOS 版本的 SDK |
...
# 2. 构建并安装 App
xcodebuild -project RodskiDemo.xcodeproj -scheme RodskiDemo \
...
推荐实践：iOS 模型优先用 `id`（accessibility-id）或 `text`（priority=1），`ocr/vision` 作为降级兜底（priority=2/3）。
