...
class ScreenRecorder:
    ...
    Args:
        ...
        overlay_step: 是否在录像上叠加步骤文字，默认 True
    """
    ...
    def set_step(self, step_text: Optional[str]) -> None:
        """更新当前步骤文字（线程安全）"""
    ...
    def _draw_step_overlay(self, img, step_text: str) -> None:
        """在帧左下角绘制半透明步骤文字条"""
        cv2 = self._cv2
        np = self._np
        h, w = img.shape[:2]
        ...
        # 截断过长文字
        max_chars = 90
        display_text = step_text if len(step_text) <= max_chars else step_text[:max_chars - 1] + "…"
        ...
        try:
            with self._mss.mss() as sct:
                ...
                while self._recording and not self._stop_event.is_set():
                    ...
                    # 叠加步骤文字
                    if self.overlay_step:
                        with self._step_lock:
                            step_text = self._current_step_text
                        if step_text:
                            self._draw_step_overlay(img, step_text)
